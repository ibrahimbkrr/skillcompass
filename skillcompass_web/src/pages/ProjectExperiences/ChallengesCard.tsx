...
const ChallengesCard: React.FC<ChallengesCardProps> = ({ value, onChange, error, completed, showInspire, inspireText, onInspire }) => {
  return (
    <AnimatedQuestionCard completed={!!completed} borderColor={completed ? '#D4A017' : '#A0AEC0'}>
      <Typography variant="subtitle1" fontWeight={600} mb={1} color="#2A4B7C">
        Projelerde karşılaştığınız en büyük zorluk neydi?
      </Typography>
      <Box display="flex" alignItems="flex-start" mb={1}>
        <TextField
          ...
          placeholder="Örneğin: Takım içi iletişim eksikliği nedeniyle proje süreci uzadı."
          error={!!error}
          helperText={error || 'Karşılaştığınız zorluğu kısaca açıklayın.'}
          size="small"
