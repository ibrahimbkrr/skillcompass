...
interface NetworkingGoalCardProps {
  value: string;
...
const NetworkingGoalCard: React.FC<NetworkingGoalCardProps> = ({ value, onChange, error, completed, showInspire, inspireText, onInspire }) => {
  return (
    <AnimatedQuestionCard completed={!!completed} borderColor={completed ? '#D4A017' : '#A0AEC0'}>
      <Typography variant="subtitle1" fontWeight={600} mb={1} color="#2A4B7C">
        Kısa vadeli bir networking hedefiniz var mı?
      </Typography>
      <Box display="flex" alignItems="flex-start" mb={1}>
        <TextField
          ...
          placeholder="Örneğin: LinkedIn üzerinden 20 yeni profesyonel bağlantı eklemek."
          error={!!error}
          helperText={error || 'Networking hedefinizi kısaca açıklayın (en az 10 karakter).'}
          size="small"
...
export default NetworkingGoalCard; 