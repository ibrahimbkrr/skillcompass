...
const ShortTermGoalCard: React.FC<ShortTermGoalCardProps> = ({ value, onChange, error, completed, showInspire, inspireText, onInspire }) => {
  return (
    <AnimatedQuestionCard completed={!!completed} borderColor={completed ? '#D4A017' : '#A0AEC0'}>
      ...
      <Box display="flex" alignItems="center" mb={1}>
        <TextField
          ...
          placeholder="Örneğin: Flutter ile 2 mobil uygulama yayınlamış bir geliştirici."
          error={!!error}
          helperText={error || '1 yıl içinde ulaşılabilir bir hedef düşünün. Rol, proje veya bir başarı olabilir.'}
          size="small"
          sx={{ background: '#F8FAFC', borderRadius: 2 }}
        />
        <IconButton onClick={onInspire} sx={{ ml: 1 }} aria-label="İlham önerisi göster">
